...
function App() {
  ...
  const [soucerText, setsoucerText] = useState('')
  ...
  }, [soucerText, targetLang, sourceLang ])
  ...
  const handleTranslate = async () => {
    ...
    try {
      const response = await fetch(`https://api.mymemory.translated.net/get?q=${soucerText}&langpair=${sourceLang}|${targetLang}`,)
  ...
  const swapTranslate = () => {
    ...
    setsoucerText(translatedText)
    setTranslatedText(soucerText)
  }
    ...
    <div className='min-h-screen bg-background flex flex-col'>
      ...
      <main className='flex-grow flex items-start justify-center px-4 py-8'>
        ...
        <div className='w-full max-w-5xl bg-white rounded-lg shadow-md overflow-hidden'>
          ...
          <div className='flex items-center justify-between p-4 border-b border-gray-200'>
            ...
            <button className='p-2 rounded-full hover:bg-gray-100 outline-none' onClick={swapTranslate} > <svg
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 '>
            <div className='p-4'>
              <textarea value={soucerText} onChange={event => setsoucerText(event.target.value)}
                placeholder='digite seu texto..' className='w-full h-40 text-lg text-textColor bg-transparent resize-none border-none outline-none'>
              </textarea>
            </div>
      ...
      <footer className='bg-white border-t border-gray-200 mt-auto'>
        <div className='max-w-5xl mx-auto px-4 py-3 text-sm text-headerColor'>
          &copy; {new Date().getFullYear()} Tradutor feito por Henzon Tadlly
        </div>
      </footer>
